...
--
-- Percobaan pertama pakai `create or replace view` GAGAL:
--   ERROR: 42P16: cannot change name of view column "omset" to "customer_phone"
-- Postgres hanya mengizinkan CREATE OR REPLACE menambah kolom baru di PALING
-- AKHIR daftar select. Karena customer_phone masuk lewat `t.*` yang posisinya
-- SEBELUM kolom hitungan (omset, modal, dst), ia "menyisip di tengah" —
-- menggeser posisi semua kolom setelahnya. Itu dianggap rename, dilarang.
--
-- Solusinya harus DROP lalu CREATE ulang. v_daily_sales dan v_monthly_pnl
-- bergantung pada v_transactions, jadi ikut ter-drop lewat CASCADE dan
-- harus dibuat ulang juga di sini (definisinya disalin persis dari
-- 0004_financial_views.sql, tidak ada perubahan logika).
--
-- CATATAN UNTUK MASA DEPAN: kalau nanti ada kolom baru ditambahkan lagi ke
-- `transactions`, ulangi pola DROP ... CASCADE + CREATE ulang tiga view ini
-- dalam satu migrasi seperti di bawah — jangan pakai CREATE OR REPLACE.
-- =============================================================================
